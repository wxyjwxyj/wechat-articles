...
def _select_highlights(items_flat: list[dict], total: int = 6) -> list[dict]:
    """多样性选取：HN/GitHub 各保底1条（ArXiv 靠分数自然竞争），剩余按分数补齐。"""
    if not items_flat:
        return []
...
def build_html(bundle: dict, preview: dict) -> str:
    ...
    highlights  = _select_highlights(items_flat, total=6)
