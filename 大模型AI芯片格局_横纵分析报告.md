...
### 3.7 白热化（2024-2026）：Blackwell、TPU v8、MI350

2024-2026 年是 AI 芯片竞争最激烈的三年。

**NVIDIA Blackwell：** 2024 年 3 月发布的 B200 是一款"怪兽"——双 die 封装（2080 亿晶体管），NV-HBI 芯片间互联 10 TB/s，FP4 峰值算力 9 PFLOPS，NVLink 5.0 带宽 1.8 TB/s。但 Blackwell 的初期遭遇了延迟——CoWoS-L 封装缺陷和液冷系统泄漏导致出货推迟，原定 2024 年底的交付推迟到了 2025 年初。

**Google TPU v8（2026 年 4 月）：** 一个标志性事件——Google 首次将训练和推理拆分为两条产品线。v8t（训练）峰值 12.6 FP4 PFLOPS，9600 芯片超算；v8i（推理）288GB HBM3e，384MB 片上 SRAM。数据也很震撼：Midjourney 切到 TPU 后推理成本从月 210 万美元降到 70 万美元以下（降 65%），Anthropic 计划到 2027 年扩展到 100 万 TPU。

**AMD MI350X：** 3nm 制程（TSMC），FP16 算力 2.39 PFLOPS，288GB HBM3e，直接对标 B200。AMD 依然面临 ROCm 生态落后的问题，但其 chiplet 架构和开放路线正在争取更广泛的开发者支持。
...
| **混战期** | 2024-2026 | 三足鼎立初现 | Blackwell、TPU v8、MI350、Cerebras |
...
TPU 是"自研芯片"这条路线最成功的案例。到 2026 年，Google 在训练中使用了约 30% TPU、在推理中使用了约 50% TPU——换句话说，Google 已经是 NVIDIA 之外最大的 AI 芯片用户。
...
华为昇腾 910B 是目前中国最可行的 A100 替代方案。7nm 工艺（SMIC），宣称 FP16 算力 ~320 TFLOPS，64GB HBM2e。软件栈自研 CANN + MindSpore。

实际问题是：软件生态极度不成熟、SMIC 拿不到 EUV、良率受限。短期内无法替代 H100/B200 级别的高端需求。但在"国产替代"的政策驱动下，中国互联网大厂已批量采购。

出口管制对比：

| 管制阶段 | 时间 | NVIDIA 对华产品 | 中国替代 |
|---------|------|----------------|---------|
| 初始管制 | 2022.10 | A100/H100 被禁 | 昇腾 910 |
| 收紧 | 2023.10 | A800/H800 被禁 | 昇腾 910B |
| 博弈 | 2026.1 | H200 获许可→被海关拦 | 昇腾 910C |

#### 云厂商自研芯片

AWS 的 Trainium 2 和 Inferentia 2、Microsoft 的 Maia 100、百度的昆仑芯 2、阿里的含光 800——几乎所有大型云厂商都有自研芯片。但这些芯片的特点是：**内部使用为主，不对外公开出售。"这多少暴露了它们的真正定位——不是来跟 NVIDIA 竞争的，而是作为谈判筹码来压低 GPU 价格的。**

#### 初创公司

Cerebras（晶圆级芯片，2026 年 5 月计划 IPO，OpenAI 200 亿 + 推理合同）、Groq（LPU 推理芯片，2025 年底被 NVIDIA 以 200 亿美元收购传闻）、SambaNova（全栈 AI 平台，2026 年 Series E 3.5 亿 +）。这些初创公司在特定场景有优势（Cerebras 的晶圆级芯片适合大模型推理），但离挑战 NVIDIA 的江湖地位还差得很远。
...
| Google TPU v5p | 2023-12 | — | 459 TFLOPS | 95GB HBM / 2.76 TB/s | ICI | ~200W | JAX/XLA | ❌ |
| Google TPU v8t | 2026-04 | — | 12.6 PFLOPS(FP4) | 216GB HBM3e / 6.5 TB/s | Boardfly | — | JAX/XLA | ❌ |
| 华为昇腾 910B | 2023 | 7nm | ~320 TFLOPS | 64GB HBM2e / — | HCCS | ~350W | CANN | ❌ |
| Intel Gaudi 3 | 2024 | 7nm | — | 128GB HBM2e / 3.7 TB/s | 24x100GbE | 600W | SynapseAI | ❌ |
| AWS Trainium 2 | 2024 | — | — | — | — | — | Neuron | ❌ |
...
第二，**CUDA 护城河正在变浅。** OpenAI Triton（Python 级 GPU 编程语言）、MLIR（多级 IR 编译器基础设施）、JAX（跨后端框架）这三驾马车，正在逐步解除开发者对 CUDA 的绑定。一个开发者写了 Triton kernel，就能在 AMD/Intel/NVIDIA 上运行——这种事在 2023 年还不现实，到 2026 年已经是主流。

第三，**出口管制在制造两个独立的 AI 芯片生态。** 中国市场和西方市场的 AI 芯片供应链正在加速分离。中国在没有 EUV 的情况下做出了昇腾 910B——底层算力差一截，但上层应用层通过算法优化弥补。长远来看，这会导致全球 AI 芯片市场分化成两个半独立的生态。

### 4.6 核心趋势

1. **低精度计算成为新战场**：FP4/FP8 训练从实验走向主流。Blackwell 的 NVFP4 可以用 4-bit 精度匹配 16-bit 的训练准确度——这意味着未来同样规模的芯片可以跑 4 倍大的模型。
2. **互联技术决定集群效率**：万卡集群时代，芯片到芯片的通信带宽正在成为比单卡算力更重要的瓶颈。NVLink 5.0 的 1.8 TB/s 和 Google Boardfly 的可重配拓扑代表了两种不同的解——私有高带宽 vs 灵活可编程。
...
2. *TPU v1: In-Datacenter Performance Analysis of a Tensor Processing Unit* (ISCA 2017) — https://dl.acm.org/doi/10.1145/3079856.3080246
3. *ImageNet Classification with Deep Convolutional Neural Networks* (AlexNet, 2012)

### 技术文档与白皮书

4. NVIDIA H100 Whitepaper — https://nvdam.widen.net/content/tdwwiwotwr/original/gtc22-whitepaper-hopper.pdf
5. NVIDIA Blackwell Technical Brief — https://resources.nvidia.com/en-us-blackwell-architecture
6. NVIDIA CUDA Wikipedia — https://en.wikipedia.org/wiki/CUDA
7. NVIDIA Tesla Wikipedia — https://en.wikipedia.org/wiki/Nvidia_Tesla
8. Wikipedia: Hopper Microarchitecture — https://en.wikipedia.org/wiki/Hopper_(microarchitecture)
9. Wikipedia: Blackwell Microarchitecture — https://en.wikipedia.org/wiki/Blackwell_(microarchitecture)

### 竞品信息

10. Google TPU Wikipedia — https://en.wikipedia.org/wiki/Tensor_Processing_Unit
11. AMD Instinct Wikipedia — https://en.wikipedia.org/wiki/AMD_Instinct
12. AMD ROCm Wikipedia — https://en.wikipedia.org/wiki/ROCm
13. Huawei Wikipedia — https://en.wikipedia.org/wiki/Huawei

### 行业分析

14. Gartner AI Inference Market Forecast (March 2026)
15. Grand View Research: AI Inference Market 2030
16. Jon Peddie Research: GPU Market Q1 2025
17. The Guardian: China blocks NVIDIA H200 (2026-01) — https://www.theguardian.com/technology/2026/jan/17/china-blocks-nvidia-h200-ai-chips-that-us-government-cleared-for-export-report
18. SemiAnalysis: China AI Chips — https://www.semianalysis.com/p/nvidias-new-china-ai-chips-circumvent
19. Forbes: AI chip market analysis
20. CNBC: Cerebras S-1 filing (2026-04)
21. Data Center Dynamics: Blackwell delay analysis
22. East Asia Forum: US-China chip export controls

### 方法论说明

本报告采用**横纵分析法（Horizontal-Vertical Analysis）**，由数字生命卡兹克提出。纵向部分追溯 AI 芯片从 2006 年 CUDA 到 2026 年 TPU v8 的完整演进史；横向部分对比当前主要 AI 芯片厂商的竞争格局；横纵交汇部分给出综合性判断和未来推演。
