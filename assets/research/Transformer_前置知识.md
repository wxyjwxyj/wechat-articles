...
> 一份零基础友好的 Transformer 入门指南。读完你就能看懂 QKV（Query-Key-Value，查询-键-值机制）、多头注意力、位置编码、FFN、
> 训练推理差异、分词机制，然后再去读横纵分析报告就会顺畅很多。
>
> **撰写时间**：2026-04-29  |  **最后更新**：2026-04-30（补充推理优化、量化、RAG、Hyperscale Lottery）
>
> 🎯 **读完这篇你能**：用自己的话解释 Transformer 每个核心组件（QKV 注意力、多头机制、位置编码、FFN、Layer Norm）做什么、为什么这么设计，并理解 Scaling Laws、FlashAttention、MoE 等关键演进方向的动机。
...
- MoE 怎么把 FFN 拆成专家团队
- INT8/INT4 量化怎么做，GGUF/GPTQ/AWQ 有什么区别
- Continuous Batching、Prefix Caching、vLLM 怎么加速推理
- 分词这个小细节为什么能决定模型天花板
...
| **Epoch** | 把整个训练数据集完整过一遍。15T tokens ÷ 8B 参数 ≈ 每 epoch ~1.6T tokens，LLaMA 3 8B 训了约 9 个 epoch（Dubey et al., 2024） |
...
| **BLEU** | 机器翻译的质量评分，0-100 分。越接近人工翻译的分数越高。Transformer 原始论文达到 28.4 分（当时最高）（Vaswani et al., 2017） |
| **Label Smoothing** | 训练技巧：不要求模型 100% 确定选某个词，留一点"不确定性"。值 0.1 表示正确答案概率=0.9，其余均分。防过拟合 |
| **vLLM** | 最流行的 LLM 推理部署框架。核心创新 PagedAttention（把 KV cache 当虚拟内存分页管理）。吞吐量比普通推理高 2-4 倍（Kwon et al., 2023） |
...
| **MFU** | 模型算力利用率。GPU 实际干活时间 ÷ 总运行时间的比例。DeepSeek V3 达到 ~70%（DeepSeek, 2024），GPT-4 约 32%（Patel & Wong, 2024） |
...
> 增加参数比增加数据更有效——73% 的额外算力应该分配给更大的模型，只有 27% 给更多数据（Kaplan et al., 2020）。

公式近似：N_opt ∝ C^0.73，D_opt ∝ C^0.27（Kaplan et al., 2020）。
...
> 参数和数据同等重要——50% 的额外算力给参数，50% 给数据（Hoffmann et al., 2022）。

公式修正为：N_opt ∝ C^0.50，D_opt ∝ C^0.50（Hoffmann et al., 2022）。最优 tokens/参数比约 **20**，而不是 Kaplan 说的 1-2（Hoffmann et al., 2022）。

**直接影响**：Chinchilla 70B 用 1.4 万亿 tokens 训练，以 1/4 的参数量超越了 GPT-3 175B（Hoffmann et al., 2022）。LLaMA 系列严格遵循 Chinchilla 公式（LLaMA 65B 用了约 22 tokens/参数）（Touvron et al., 2023），13B 版本在多数 benchmark 上超越 GPT-3 175B。
...
LLaMA 3 8B 用了约 15T tokens——约 **1,875 tokens/参数**，远超 Chinchilla 最优的 ~20（Dubey et al., 2024）。为什么？因为高质量预训练数据几近耗尽，与其找更多数据，不如用更多 epoch 重复训练——这就是"over-training"。小模型尤其受益于这种方式。
...
**RMSNorm**：LayerNorm 的简化版。研究发现均值偏移不重要，只做 scaling 就够了，节省约 7% 的计算（Zhang & Sennrich, 2019）。
...
**结果**：输出和原始 attention **完全一样**（精确到 bit），但速度快 2-4 倍，内存从 O(n²) 降到 O(n)（Dao et al., 2022）。这是一个"工程创新超越算法创新"的经典案例。

### FlashAttention v2/v3

- **v2（2023）**：改进并行化策略，A100 FP16 达到 ~230 TFLOPS（万亿次浮点运算/秒，约 70% 理论峰值）（Dao, 2023）
- **v3（2024）**：深度适配 H100 的 Hopper 架构特性（WGMMA 指令、TMA 异步拷贝），FP16 达到 740 TFLOPS、FP8 达到 ~1.2 PFLOPS（千万亿次/秒）（Dao, 2024）
...
所有 attention head **共享同一套 K、V**，只保留独立的 Q。KV Cache 降为原来的 1/h（h 是头数，比如 8 头就降为 1/8）（Shazeer, 2019）。

**代价**：所有头用同样的 K、V，注意力多样性降低，有精度损失。GPT-4 和 PaLM 用了它。

### GQA（Grouped-Query Attention, Ainslie 2023）

折中方案：把 h 个头分成 g 组，每组共享一套 K、V。比如 8 头分 4 组 → KV cache 降为 1/4（Ainslie et al., 2023）。精度损失远小于 MQA。LLaMA 3、Qwen、Mistral 全用 GQA。

### MLA（Multi-Head Latent Attention, DeepSeek 2024）

不开玩笑的创新：发现多个 head 的 K、V 之间有大量重叠信息，把 KV 投影矩阵做**低秩分解**（W_K ≈ W_K_down × W_K_up），信息空间远小于参数空间。KV Cache 降 ~10 倍，精度匹敌 GQA 甚至更好（DeepSeek, 2024）。
...
- **稀疏激活**：虽然模型总参数巨大（DeepSeek V3 671B），但每个 token 只激活 5.5%（37B）（DeepSeek, 2024）
...
**PagedAttention（vLLM）**：KV cache 的"虚拟内存"。不再把 KV cache 作为一整块连续显存分配，而是切成固定大小的 page，按需分配。解决了显存碎片和预留过大的问题，吞吐量提升 2-4 倍（Kwon et al., 2023）。
...
**Multi-Token Prediction（MTP, DeepSeek V3）**：每步不只预测下一个 token，还预测下下个 token。第二个 token 接受率 85-90%（DeepSeek, 2024），推理时可以用来做 speculative decoding。

### Continuous Batching（连续批处理）

传统批处理是"等一批请求到齐→一起推理→返回结果"。如果有一个请求来得慢，所有请求都等着——浪费时间。Continuous Batching 把推理过程拆成更细的粒度：每个请求的 attention 计算步长不一样，先算完的先返回，新请求随时插入空位。vLLM、TensorRT-LLM 等推理引擎都靠这个技术把吞吐量提了 2-5 倍（Kwon et al., 2023）。

### Prefix Caching（前缀缓存）

很多请求共享相同的 prompt 前缀（如 system prompt、few-shot 示例）。Prefix Caching 把前缀部分算好的 KV cache 缓存下来，新请求来时直接复用，不用重新计算。效果取决于前缀命中率——在线客服（所有请求 system prompt 相同）命中率极高，随机用户 query 则低很多。vLLM 的 Automatic Prefix Caching 自动管理这个缓存。

### vLLM 是做什么的

vLLM 是当前最主流的推理框架，核心贡献是把上面几件事整合到一起：PagedAttention 解决显存碎片、Continuous Batching 提高吞吐、Prefix Caching 减少冗余计算。加上量化支持（后面会讲），一张 A100 80GB 可以同时服务数十个 LLaMA 70B 请求。它的思路是**"不降低模型质量的前提下，把硬件用得更狠"**。
...
DeepSeek V3 是业界首个大规模 FP8 训练的 SOTA 模型。核心技巧是**块级量化**——不全局用一套缩放因子，而是把 tensor 分块，每块独立缩放，避免异常值污染整块。结合 DualPipe 通信计算重叠，MFU（模型算力利用率）达到 ~70%（DeepSeek, 2024）。

### 推理量化：INT8 与 INT4

训练之外，**推理时也可以用更低精度**——而且做法完全不同。

训练量化是为了减少计算开销、省显存。推理量化是为了**把模型塞进更便宜的硬件、跑得更快**。一张 80GB 的 A100 只能塞一个 FP16 的 70B 模型，量化到 INT4 后变成 ~35GB，一张 48GB 的 L40S 就能跑，成本降一半。

**权重量化（Weight-only Quantization）**：只压缩模型参数，不压缩计算过程。计算时先把 INT4 权重解回 FP16，再算。好处是简单、显存省得多，坏处是计算速度提升有限（因为解压也要时间）。代表方案：**GGUF**（llama.cpp 用的格式，消费级显卡/CPU 都能跑）、**GPTQ**（逐层校准量化，精度保持好，但不适合 dynamic batching）、**AWQ**（按 activation 分布分配量化精度，敏感 channel 多留几位，效果跟 GPTQ 相当但硬件友好）。

**KV Cache 量化**：上一节讲过 KV cache 占推理显存大头。把 KV cache 从 FP16 降到 INT8，显存直接减半。精度损失极小（因为 attention 对噪声容忍度高），几乎所有生产推理引擎都默认启用。

**训练后量化（PTQ） vs 量化感知训练（QAT）**：PTQ 是模型训完后直接压——拿几百条校准数据算一下缩放因子就行，快但大模型精度可能掉 1-2%。QAT 在训练阶段就模拟量化误差，让模型学会适应低精度——精度几乎不掉，但需要重训或 LoRA 微调。主流做法是先用 PTQ 快速验证，精度不够再上 QAT。

量化不是"免费午餐"：INT4 比 INT8 省一半显存，但精度掉得更快，尤其是小模型（<7B）和小语种。70B+ 模型对量化的容忍度更高，因为参数冗余多。
...
2023 年斯坦福提出的更简单方案（Rafailov et al., 2023）：**不需要单独训练奖励模型，直接用好/坏回答的对比来优化模型。**
...
Mamba 在 1-3B 参数级别证明了自己——跟同规模的 Transformer 性能相当，但推理吞吐高 5-6 倍，因为不需要存 KV cache（Gu & Dao, 2023）。
...
Qwen3.5 把 75% 的 attention 层换成了 Gated DeltaNet（一种线性注意力），剩下的 25% 保留标准 attention 用于关键推理步骤（Qwen Team, 2025）。Jamba、Zamba 也在走类似路线。
...
- Dubey et al., "The Llama 3 Herd of Models" (2024) — https://arxiv.org/abs/2407.21783
- Kwon et al., "Efficient Memory Management for Large Language Model Serving with PagedAttention" (2023) — vLLM 推理框架的核心，吞吐量提升 2-5 倍 — https://arxiv.org/abs/2309.06180
- Gu & Dao, "Mamba: Linear-Time Sequence Modeling with Selective State Spaces" (2023) — 选择性 SSM 的突破，推理吞吐高 Transformer 5-6 倍 — https://arxiv.org/abs/2312.00752
- Patel & Wong, "AI Data Center Scaling — 2025 Outlook" (SemiAnalysis, 2024) — GPT-4 MFU 约 32% 的来源
- Zhang & Sennrich, "Root Mean Square Layer Normalization" (2019) — RMSNorm 简化 LayerNorm，节省约 7% 计算 — https://arxiv.org/abs/1910.07467
- Qwen Team, "Qwen3.5 Technical Report" (2025) — 混合架构，75% attention 层替换为 Gated DeltaNet
