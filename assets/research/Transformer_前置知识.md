...
传统批处理是"等一批请求到齐→一起推理→返回结果"。如果有一个请求来得慢，所有请求都等着——浪费时间。Continuous Batching 把推理过程拆成更细的粒度：每个请求的 attention 计算步长不一样，先算完的先返回，新请求随时插入空位。vLLM、TensorRT-LLM 等推理引擎都靠这个技术把吞吐量提了 2-5 倍。
