...
**核心矛盾**：模型越来越大（405B、671B），但推理设备的内存带宽和容量没跟上。所有推理优化——量化、KV cache 压缩、并行策略、投机解码——本质上都是在跟这个矛盾周旋。
...
**自回归生成**：Transformer 推理是逐 token 生成的——输出 "hello" 后，把 "hello" 拼回输入再预测下一个 token。这意味着无法完全利用 GPU 的并行计算能力，因为第二步必须等第一步完成。
...
> - [Transformer 训练](./Transformer_训练横纵分析报告.md) — 训练只做一次，推理跑无数次，两者的优化目标截然不同
...
**专家并行（Expert Parallelism, EP）**：MoE 模型专用。把 different experts 分配到不同的 GPU 上，token 在哪张 GPU 上就由哪张 GPU 上的 expert 处理。问题是 all-to-all 通信——token 可能需要从 GPU A 跳到 GPU B 去处理，跳来跳去通信开销极大。DeepSeek V3 的 DualPipe 本质上就是让通信和计算重叠，掩盖这个开销。
...
这意味着"推理部署"这个职能在 AI 公司内部的权重会持续上升。它不再是一个支撑团队，而是直接影响产品形态和体验的核心工程团队。2026 年各大 AI 公司的基础设施招聘也印证了这一点——推理工程师的缺口远大于训练工程师。
