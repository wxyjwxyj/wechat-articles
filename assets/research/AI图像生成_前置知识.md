...
2022 年是 AI 图像生成进入大众视野的元年：DALL-E 2 和 Midjourney 让"文字变图片"从一个研究实验变成了人人可用的产品。到 2026 年，AI 生成图片已经无处不在——从社交媒体头像到电商产品图到电影概念设计。
...
**DDPM（Denoising Diffusion Probabilistic Models）** 是 2020 年 Ho 等人发表的论文，重新引起了学界对扩散模型的兴趣。它是第一个在图像生成上系统超越 GAN 的扩散模型——训练稳定、生成质量高。这个工作奠定了后续所有扩散模型（DALL-E 2、Stable Diffusion、Midjourney）的技术基础。

扩散模型之所以取代了 GAN，是因为训练更稳定、多样性更好。GAN 需要同时训练"生成器"和"判别器"两个网络对抗博弈，极易失败。扩散模型只需要训练一个去噪网络。

### 潜在扩散（Latent Diffusion）—— Stable Diffusion 的核心

直接在像素空间做扩散太慢（一张 1024×1024 的图就是 300 万像素）。Stable Diffusion 的聪明之处：先用一个 VAE（Variational Autoencoder，变分自编码器）把图片压缩到小巧的"潜在空间"（latent space），在压缩后的空间里做扩散，最后再把结果解压回高清图。

这就像：写一部长篇小说的提纲（潜在空间），而不是写完整本书再修改。

### DiT（Diffusion Transformer）

2024 年开始普及的新架构。之前扩散模型的核心网络是 U-Net（一种编码-解码结构）。DiT 用 Transformer 替代 U-Net，效果更好、扩展性更强。Flux（2024 年 8 月）是第一个大获成功的 DiT 图像生成模型。
...
- **ControlNet**：给生成过程加额外的控制条件——比如提供一张"人物姿势骨架"图，让生成的图片完全按照这个姿势来
- **LoRA**：用少量图片微调模型，让模型学会生成特定风格或特定人物
