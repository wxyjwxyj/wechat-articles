...
> 🎯 **读完这篇你能**：判断一个场景该不该引入 Agent，能画出 Agent 的四件套架构（LLM → 工具调用 → 记忆 → 自主循环），并解释每层解决什么问题。
...
### WebGPT——Agent 的雏形（2021）

在 Agent 2023 年爆发之前，有一个常被忽略但很重要的前奏：OpenAI 在 2021 年底发布了 **WebGPT**（Nakano et al., 2021）。WebGPT 让 GPT-3 通过浏览器搜索来回答问题——用 Bing 搜索、浏览网页、提取关键信息、综合成最终回答。它已经有了 Agent 的三个核心要素：**工具**（浏览器）、**规划**（搜索策略：先搜什么、点哪个链接、什么时候停）、**记忆**（记住已经看过的页面内容）。WebGPT 在事实准确性上比纯 GPT-3 高了 30% 以上。只不过当时还没有"Agent"这个热词，它被归类为"检索增强"——但回头看，它已经是一个初级 Agent 了。
...
**2023 年 6 月，OpenAI 首次在 GPT-4 中引入 Function Calling**（OpenAI, 2023），这是 Agent 历史上的一个关键节点。在此之前，让 LLM 调用工具需要写复杂的 prompt hack，非常不可靠。Function Calling 让工具调用变成模型的原生能力。
...
### Reflexion——让 Agent 从失败中学习

ReAct 有一个局限：如果 Agent 在某一步走错了方向，它可能在循环里打转——反复推理和查询但始终没有接近答案。2023 年 Shinn et al. 提出的 **Reflexion** 解决了这个问题：Agent 完成任务后，用语言反思自己的表现（"我哪里做错了？下次应该怎么改进？"），把反思结果存入长期记忆，下次遇到类似任务时自动调整策略。在 HumanEval 编程基准上，Reflexion 让 GPT-4 的通过率从 80% 提升到 91%。这相当于给了 Agent 一个"从犯错中学习"的能力。
...
最简单的规划方式：让模型"一步一步思考"。Google 在 2022 年发现，在 prompt 里加一句"让我们一步一步来"，复杂推理的准确率能提升 30% 以上（Wei et al., 2022）。
...
就是当前对话的上下文窗口。Claude 支持 200K token（Anthropic, 2024），Gemini 支持 1M+（Google, 2024），相当于能一次性"记住"几百页文档。但 Agent 一旦开始新任务，旧对话就忘了。
...
### 一个经典案例：Generative Agents 的记忆架构

2023 年斯坦福的 "Generative Agents" 论文（Park et al., 2023）把 25 个 AI Agent 放进一个模拟小镇，让它们自主生活。这篇论文最重要的贡献之一是提出了一套完整的 Agent 记忆架构——**观察 → 记忆 → 反思 → 规划**的四层循环：

1. **观察（Observe）**：Agent 感知周围发生的事——"小明在厨房做早餐"、"小红走进客厅"
2. **记忆流（Memory Stream）**：每个 Agent 维护自己的记忆流，把观察到的事件按时间顺序存起来。这是一个庞大的记录，旧记忆会随时间衰减
3. **反思（Reflect）**：Agent 定期对记忆流做高层反思——不是逐条翻旧账，而是提炼模式。比如从过去一周的记忆中提炼出"小明每天早上都会煮咖啡"。这个反思结果本身也作为新记忆存回去，供未来决策用
4. **规划（Plan）**：Agent 基于当前记忆和反思结果，生成当天的行动计划——"先去厨房煮咖啡，然后去办公室"

这个架构深刻影响了后来几乎所有 Agent 系统的记忆设计。它的核心洞察是：**记忆不只是"存下来"，还需要"定期反思提炼"**——就像人不是逐条背诵过去，而是从中抽象出经验和模式。
...
多 Agent 系统最迷人的特性是**涌现**——多个简单的 Agent 各自执行自己的任务，却在整体上表现出意想不到的复杂行为。2023 年 Stanford 的 "Generative Agents" 论文把 25 个 Agent 放进一个模拟小镇，它们自主社交、组织派对、传播新闻（Park et al., 2023），没有人写脚本规定它们"应该举办派对"——这个行为是自发涌现出来的。这就是多 Agent 比单 Agent 更有想象空间的原因。
...
2025 年底诞生的开源项目，到 2026 年 4 月 GitHub Star 数突破 30 万（社区估算），超越 Linux 和 React 成为 GitHub 软件类星标历史第一。理念很直接：在你自己的电脑上跑一个 Agent 系统，让它自主管理文件、浏览网页、操作各种应用——不需要经过 App 的 UI。"用户不想用更多 App，他们想要一个能替自己操作所有 App 的 Agent"。之后 Hermes Agent（2026 年 3 月，Nous Research）以"持久记忆+自我进化"为差异化迅速崛起，10 万+ Stars（社区估算），形成 OpenClaw 追求"广度"、Hermes Agent 追求"深度"的双足鼎立格局。
...
目前人类在 SWE-Bench 上的解决率约 85%（Jimenez et al., Princeton, 2024），最强的 Agent（Claude + Agent 框架）约 49%（社区估算），说明 Agent 还有很大提升空间。
...
| **AutoGPT** | 个人项目 | 2023 年引爆 Agent 概念的项目，183k stars（社区估算），自主任务分解 |
...
按目前主流 API 价格（每百万 token 约 $3-15，社区估算），一个复杂 Agent 任务可能消耗 50 万-200 万 token，单次成本 $1.5-$30（社区估算）。这就是 Agent 商业化最大的瓶颈——**用得起 vs 用得值**之间的平衡。
...
- **Function Calling and Other API Updates** (OpenAI, 2023) — GPT-4 首次引入原生 Function Calling 能力，是 Agent 工具调用的关键节点 — https://openai.com/index/function-calling-and-other-api-updates/
- **SWE-bench: Can Language Models Resolve Real-World GitHub Issues?** (Jimenez et al., Princeton, 2024) — AI 编程事实标准评测，用 2294 个真实 GitHub Issue 衡量模型修 Bug 能力 — https://arxiv.org/abs/2310.06770
