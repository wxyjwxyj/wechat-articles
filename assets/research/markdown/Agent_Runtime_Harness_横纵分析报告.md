...
> **📚 关联报告**
> - [AI Agent](./AI Agent_横纵分析报告.md) — Runtime/Harness 是 Agent 的运行时基础设施，Agent 报告讲"是什么"，本篇讲"怎么跑起来"
...
**Anthropic 的入场：Managed Agents（2026.04.10）** 是整个 Harness 叙事里最戏剧性的转折。在 OpenAI 定义了 Harness 的话语权后不到两周，Anthropic 抛出了自己的答案——一个全托管的 Agent 云平台。

Claude Managed Agents 的架构核心是 **"解耦大脑与双手"**（Decoupling the brain from the hands）。传统理解里 Agent = LLM + 工具调用，Managed Agents 把这条链路拆成三层：
- **Agent**（大脑）：定义目标、规划步骤、决策分支，管"想什么"
- **Environment**（双手）：内置沙箱，提供文件系统、终端、浏览器、Git 操作能力，管"做什么"
- **Session**（执行管道）：持久化状态、自动重试、审计日志，管"怎么持续跑"

这与 OpenAI Agents SDK 的根本差异不在功能清单，而在**设计哲学**：OpenAI 把 Harness 视为模型的"附属控制面"，Anthropic 把 Harness 视为模型的"独立执行层"——Environment 与模型之间通过 API 边界解耦。好处是 Agent 崩溃不影响 Environment 状态，代价是多了跨进程通信的开销。

**Vault（凭证保险库）** 是 Anthropic 在 Harness 安全上的独特创新——Agent 调用外部 API 时不直接接触 token，所有凭证存在 Vault 中，Agent 只拿一个临时授权句柄。这个设计回答了 Harness 安全的核心矛盾：Agent 既要能自主调用外部服务，又不能持有敏感凭证。

**并行 Agent 团队**：Managed Agents 支持多个子 Agent 同时执行独立子任务，主 Agent 负责协调和汇总。Anthropic 官方给出的实验数据是"完成率和稳定性显著提高"。

发布后行业反应两极分化。支持者认为这是"Agent 从概念到生产的 10 倍加速器"（将数月的 Agent 基建压缩到几天）。批评者指出了**厂商锁定风险**：Managed Agents 的 Environment 层与 Anthropic 云深度绑定，Agent 离开这个环境无法运行——这跟 OpenAI 把 Harness 做进 SDK 的开放策略形成了鲜明对比。

**海通国际研报**明确评价"头部大模型公司入局 Agent Infra，标志 AI 行业向系统层深化"。在此之前，Agent Infra 是云厂商和创业公司的地盘，OpenAI 和 Anthropic 的入场意味着 Harness 不再是谁都能做的"胶水代码"，而是模型公司手里的一张牌。

**Microsoft Foundry Hosted Agents（2026.04 公测）** 几乎跟 Anthropic 同期发布。核心卖点是"零成本闲置"（Agent 不运行时不计费）、企业身份治理（Entra ID 集成、Role-based Access Control）和持久状态（Agent 中断后从断点恢复）。三条差异化线索：比 Anthropic 更"企业合規导向"，比 AWS/Google 更"Azure 生态绑定"，比自建更"开箱即用"。

**其他值得关注的变化**：
- **Cloudflare Agent Cloud**（2026）正式入局，利用全球边缘节点为 Agent 提供低延迟执行环境
- **Google Gemini Enterprise Agent Platform**（2026）把 Agent Harness 与管理控制台、IAM 权限、审计日志打通，主打企业合规
- **Cursor 3.2**（2026.04）创造性地把自己定位为"Agent Execution Runtime"——IDE 不只是写代码的工具，也是 Agent 运行的环境
- **OpenAI + AWS 合作**：AWS Bedrock 上部署 OpenAI 的 Stateful Agent Runtime，两个竞争对手在 Harness 层的互利合作
...
## 四、横向分析：2026 年 5 月 Agent Runtime 竞争图谱

### 4.1 核心产品对比

| 产品 | 发布方 | 发布时间 | 核心定位 | 沙箱技术 | MCP 支持 | 编排方式 | 开源 |
|------|--------|---------|---------|---------|:-----:|---------|:---:|
| **Claude Managed Agents** | Anthropic | 2026.04 | 全托管 Agent 云平台 | 内置沙箱（文件/终端/浏览器） | ✅ | Agent→Environment→Session | ❌ |
| **OpenAI Agents SDK + Harness** | OpenAI | 2025.03→2026.04 | 模型原生控制面 | API层沙箱 | 部分 | Handoff + Symphony | ❌ |
| **Microsoft Foundry Hosted Agents** | Microsoft | 2026.04 公测 | 企业合规 Agent 容器 | 专用容器 + Entra ID | ✅ | 托管编排 + 持久状态 | ❌ |
| **AWS Bedrock AgentCore** | AWS | 2025.07 | 云托管 Agent 运行时 | Firecracker microVM | ✅ | 多框架兼容 | ❌ |
| **Gemini Enterprise Agent Platform** | Google | 2026 | 企业级 Agent 平台 | gVisor (GKE) | ✅ | IAM + 审计集成 | ❌ |
...
| **Cloudflare Agent Cloud** | Cloudflare | 2026 | 边缘 Agent 运行时 | 边缘隔离 | ✅ | 全球边缘节点 | ❌ |
...
| Anthropic: "Scaling Managed Agents: Decoupling the brain from the hands" | anthropic.com/engineering/managed-agents (2026.04) | 2026-05-01 |
| "Claude 4.6 托管代理详解" | developer.aliyun.com/article/1727149 | 2026-05-01 |
| Anthropic: "Claude Managed Agents: get to production 10x faster" | claude.com/blog/claude-managed-agents (2026.04) | 2026-05-01 |

### 产品与技术来源

| 来源 | URL | 访问时间 |
|------|-----|---------|
| Anthropic Claude Managed Agents | platform.claude.com/docs/en/managed-agents (2026.04) | 2026-05-01 |
| Microsoft Foundry Hosted Agents | devblogs.microsoft.com/foundry (2026.04) | 2026-05-01 |
| Cloudflare Agent Cloud | cloudflare.com (2026) | 2026-05-01 |
| Google Gemini Enterprise Agent Platform | cloud.google.com (2026) | 2026-05-01 |
| Cursor 3.2 Agent Runtime | futurumgroup.com (2026.04) | 2026-05-01 |
...
*本文是横纵分析系列的第 26 篇报告。方法论：横纵分析法——纵向沿时间轴追溯 Agent Runtime 从 AutoGPT 到 Anthropic Managed Agents 的四年演变，横向在 2026 年 5 月对比沙箱、护栏、MCP 网关、可观测性方案及全托管 Harness 平台的竞争格局。*
