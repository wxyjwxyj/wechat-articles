...
> 这份文档为零基础读者准备。如果你已经熟悉对齐、RLHF（Reinforcement Learning from Human Feedback，基于人类反馈的强化学习）、红队测试等概念，可以直接翻阅主报告。

> 🎯 **读完这篇你能**：解释对齐问题的本质，说出 RLHF 的三步流程（偏好数据 → 奖励模型 → PPO 优化），并能区分 RLHF 和 DPO 的差异及各自适用场景。
...
ChatGPT 和 Claude 早期版本都用这个方法（Ouyang et al., 2022）。缺点是流程复杂、计算成本高。
...
### DPO（Direct Preference Optimization，直接偏好优化）化）

RLHF 的简化版（Rafailov et al., 2023）——去掉了"训练奖励模型"和"PPO 优化"两步，直接把"人类喜欢 A 胜过 B"这个信息编码成训练信号。效果接近 RLHF，但训练流程简单得多。2026 年已成为开源社区的事实标准。
...
Anthropic 提出的替代方案（Bai et al., 2022）——不让人类一个一个比较回答，而是给 AI 写一本"宪法"（一组原则），让 AI 自己判断什么回答符合宪法。
...
Anthropic 在 2025 年发布的一种安全防御技术——在输入端和输出端各部署一个分类器，用宪法原则而非固定关键词来判断内容是否安全。测试结果：越狱成功率从 86% 降到 4.4%，误伤（把正常请求当成有害）仅增加 0.38%（Anthropic, 2025）。

### System Card（系统卡片）

模型发布时附带的安全评估文档——系统性地列出模型在哪些方面可能不安全、做过什么测试、发现了什么问题、怎么修的。GPT-4 发布时首次大规模使用这个格式（OpenAI, 2023），后来成为行业惯例。

### RSP（Responsible Scaling Policy，负责任扩展政策）

Anthropic 在 2023 年提出的一种安全管理框架（Anthropic, 2023）——借鉴生物安全等级（BSL-1 到 BSL-4），把 AI 系统按能力分为 ASL-1 到 ASL-4 四个等级，每个等级对应不同的安全要求。能力越强的模型，安全标准越严。这是业界首个结构化的 AI 安全分级框架。

### EU AI Act（欧盟人工智能法案）

2024 年 8 月生效的全球首部全面 AI 监管法规（European Union, 2024）——把 AI 系统按风险分为四级（不可接受→禁止、高风险→评估要求、有限风险→透明度要求、最低风险→不监管）。虽然只适用于欧盟，但其分级框架已成为全球 AI 治理的参考标准。
...
| **SSI**（Safe Superintelligence Inc.） | Ilya Sutskever | 2024 年创立，专注"安全超智能"，融资 10 亿美元（社区估算） |
| **CAISI**（原 US AISI） | 美国政府（NIST 下） | 官方安全评估机构，2025 年从"安全"转向"标准与创新" |

### 几个重要的组织背景术语

- **capped-profit（利润上限）**：OpenAI 在 2019 年从非营利转型时采用的结构——投资者回报有上限（最初设定为 100 倍）（OpenAI, 2019），超过上限的利润归非营利主体所有。这是 OpenAI "既要融资又要安全使命"的折中方案。
...
- **Constitutional Classifiers: Rapid and Robust Safety Guardrails** (Anthropic, 2025) — 在输入端和输出端部署宪法分类器，越狱成功率从 86% 降至 4.4%，误伤仅增加 0.38% — [Anthropic Blog](https://www.anthropic.com/news/constitutional-classifiers)
