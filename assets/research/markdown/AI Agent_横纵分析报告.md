...
> 🎯 **读完这篇你能**：判断一个场景该不该用 Agent，理解 Agent 和普通 LLM 的本质区别在于"自主循环决策"而非"一问一答"，能评估一个 Agent 系统的工具设计、记忆策略和规划机制是否合理。

---

### 阅读指南

**如果你只有 5 分钟**：读[一句话定义](#一一句话定义) + [横纵交汇洞察](#五横纵交汇洞察)的最后一部分（"Agent 会走到哪里"的三个未来剧本）。你能在 5 分钟内把握 Agent 的本质和未来方向。

**如果你想理解 Agent 但不碰技术细节**：读[技术背景](#二技术背景)的 2.1（核心公式）和 2.3（为什么现在才火）+ [纵向分析](#三纵向分析)每个阶段的标题和阶段末的总结句 + [横向分析](#四横向分析)的产品形态对比和战略分组部分。约 25 分钟。

**如果你是 Agent 开发者/研究者**：直接跳[横向分析](#四横向分析)的框架阵营对比和基准测试表现。纵向分析的 3.4（2023 Agent 元年：AutoGPT、BabyAGI）和 3.7（2026：Agent 编程范式的全面确立）是两个关键转折点，[横纵交汇洞察](#五横纵交汇洞察)中关于 Agent 核心矛盾的讨论值得精读。
...
**1983 年，卡内基梅隆大学的 John Laird、Allen Newell 和 Paul Rosenbloom 创建了 SOAR（State, Operator And Result）认知架构**。这是第一个可计算的通用智能体架构，试图用符号系统模拟人类的认知过程。SOAR 的核心思想是：智能体在状态空间中搜索，选择算子（Operator）作用于当前状态，产生新状态，循环往复直到目标达成。今天 ReAct 模式的"思考→行动→观察→思考"循环，和 SOAR 的问题空间搜索是同构的——只不过当年的算子是手工定义的符号规则，而今天的算子是 LLM 动态生成的。
...
另一个常被忽略的前奏工作是 2022 年的 **SayCan**（Google Robotics, arXiv:2204.01691）。SayCan 让 LLM 为机器人规划行动——LLM 说"我要做什么"，机器人说"我能做什么"，两者协商出可行的方案。这正是后来的"LLM + 行动系统"的 Agent 架构，只不过行动系统不是软件工具而是物理机器人。
...
### 3.2 爆发前夜（2017-2022）：Transformer 激活 Agent 范式
...
**2022 年 1 月，Google 的 Wei et al. 发表了 Chain-of-Thought Prompting（CoT）。** 从今天的角度看，CoT 的意义远不止"提升推理准确率"——它第一次展示了：**LLM 的推理可以通过 prompt 工程被引导、被结构化的。** Agent 的"思考过程"不再是黑箱，而是可以被程序化控制的。
...
**10 月，Anthropic 发布 Claude Computer Use。** **这是 2024 年最重要的 Agent 事件。** Anthropic 展示了 Claude 3.5 Sonnet 操控计算机界面的能力——看屏幕截图、移动光标、点击按钮、输入文字。不是通过 API，而是像人一样"用眼睛看、用手操作"。由此，Agent 可以操作任何软件，无论它有没有 API。这个突破打开了 Agent 在 GUI 时代的新范式。
...
**OpenClaw 带起来的浪潮还没退，第二个现象级项目 Hermes Agent 杀了进来。** 2026 年 3 月，Nous Research 发布了 Hermes Agent——同样定位为开源个人 Agent 平台，但核心卖点完全不同：**持久记忆和自我进化。** OpenClaw 每次重启会丢失上下文，Hermes Agent 用 FTS5 记忆系统把"每次对话的历史"持久化存储，Agent 越用越了解你的习惯和偏好。它的自我进化机制——Agent 能反思自己的过往行为并调整策略——是 OpenClaw 没有的功能。

到 2026 年 4 月，Hermes Agent 在不到两个月内冲到 10 万 Stars，成为 OpenClaw 之后增长最快的 AI Agent 开源项目。中国社交网络出现"养龙虾不如养马"的讨论。但对标 OpenClaw 的快速膨胀也带来了同样的烦恼——功能迭代太快、稳定性不足、以及社区关于"代码抄袭"的争议（2026 年 4 月被指控抄袭中国团队的架构设计）。

Hermes Agent 和 OpenClaw 的竞争定义了 2026 年 Agent 品类的核心分化：**OpenClaw 追求"广度"（什么都能操作），Hermes Agent 追求"深度"（越用越懂你）。** 两个项目的 Star 数加起来超过 40 万，说明用户对"个人 AI Agent"的需求不是一时热度——它是真实存在的。
...
**同样在 4 月，Anthropic 发布 Claude Managed Agents（托管智能体服务）**——开发周期从数月缩短到数天。由此，Anthropic 从"提供模型"直接进入了"提供 Agent 服务"的领域。行业反应激烈：**"Anthropic 亲自下场，又一批 Agent 创业公司死掉了。"** 同月，Anthropic 还发布 Agent Skills 开放标准，并宣布 Enterprise 从 per-seat 转向按用量计费——重度用户成本可能翻倍，被视为"AI 免费时代结束"的又一标志。
...
**也是在 4 月，中国否决了 Meta 以 $20 亿收购 Manus 的交易。** 由此可见，AI Agent 已经被纳入战略资产范畴，国际资本流动受到管控。
...
| **Hermes Agent** | Nous Research | 2026-03 | 100,000+ | ✅ | 自我进化+持久记忆 Agent |
...
**LangChain 的生态霸权与复杂度争议。** LangChain 是 Agent 框架里最"重"的——它提供了 700+ 集成、完整的工具链（LangSmith 监控、LangServe 部署、LangGraph 状态图），但也因此背负了大量批评。最典型的一句吐槽："LangChain 在 OpenAI 完成 80% 工作的地方加了 200% 的复杂性。" 社区中很多开发者转向更轻量的方案（直接调 OpenAI API + 写几十行 Python）。但 LangChain 的生态壁垒正在形成：新工具发布时，支持 LangChain 往往是最先做的集成。
...
| **Hermes Agent** | Nous Research | 2026-03 | 自我进化+持久记忆 Agent | 免费开源 |
...
**值得关注的新玩家**：Sycamore Labs（2026.03）以 $65M 种子轮（Coatue + Lightspeed 联合投资）打造"企业 Agent 操作系统"，定位在 Agent 治理和编排层——这是 Agent 赛道最大的种子轮，反映了资本对"Agent 基础设施"的需求。ServiceNow AI Agents（A2A 协议首批企业集成）代表企业级 ITSM 巨头全面拥抱 Agent 范式。
...
**Hermes Agent vs OpenClaw：2026 年最激烈的 Agent 之争。** Hermes Agent 的支持者最常提的三个词——"记忆"、"进化"、"懂我"。用户切换的理由很直接："OpenClaw 每次都像第一次见你，Hermes Agent 记得你上个月说过的话。"吐槽集中在：自我进化偶尔会产生"学习错误"——Agent 从一次失败的交互中学会了不该学的行为模式。架构抄袭争议（被指控抄袭中国团队 EvoMap）也让部分技术社区对 Nous Research 的信任打了折扣。用户评价："Hermes Agent 是理想主义的产品——但它现在还只是个原型。"
...
- **Hermes Agent（2026-03）** 是 OpenClaw 之后增长最快的 Agent 项目——两个月内 10 万 Stars。它的差异化打得很准：OpenClaw 没做好的"记忆"和"自我进化"是 Hermes Agent 的核心卖点。但它和 OpenClaw 走了同一条路——个人 Agent 操作系统——意味着它直接和 OpenClaw 抢用户，而不是创造新品类。这种"正面刚"的竞争策略短期内能借 OpenClaw 的知名度获益，但长期要看它能不能在"记忆"这个单一差异化上建立足够宽的护城河。

### 5.3 优势与劣势的历史根源

**Agent 的核心优势——"自主性"——可以追溯到 ReAct 论文（2022）。** ReAct 的"推理+行动"循环是所有现代 Agent 的底层设计模式。所有的 Agent 框架、产品、基准测试，都是在优化这个循环的某个环节。
...
**2026 年最大的格局变化：Anthropic 从模型公司变成了 Agent 平台公司。** 从 Claude Agent SDK（2025.09）到 Cowork/Dispatch（2026.03）再到 Managed Agents（2026.04），Anthropic 在半年内完成了从"卖模型"到"卖 Agent 服务"的转型。这个决策的逻辑很清楚：Agent 消耗的 token 是普通聊天的 10-50 倍，做 Agent 产品比单纯卖 API 能 capture 更多价值。但对 Agent 创业公司来说，最大的模型供应商之一变成了直接的竞争对手。行业把这称为 **"平台挤压"**——当模型厂商自己做 Agent 时，依赖该模型的 Agent 创业公司的生存空间急剧缩小。
...
这场关于控制权的博弈，最终的决定因素可能不是技术能力，而是**信任**——人类多大程度上信任 Agent 替自己做决策。而信任，说到底是个经验问题，不是技术问题。就像自动驾驶需要累积足够多的安全行驶里程，Agent 也需要积累足够多的"无事故任务次数"，才能真正融入我们的日常生活。
...
#### Q1: Tool Calling 和 Agent 的本质区别是什么？

考察对 Agent 概念的理解深度，而非复述定义。

**核心区别**：Tool Calling 是"模型能调用函数"，Agent 是"模型能自主决定何时、如何、按什么顺序调用函数，并在失败时调整策略"。Tool Calling 是一次性的——用户说"查一下北京天气"，模型调一个天气 API。Agent 是多步自主的——用户说"帮我安排去北京的旅行"，模型自己拆成查天气、比机票、找酒店、排行程，中间某个 API 失败会换策略而不是直接报错。

更深一层：Agent 的核心差异化不是"有工具"，而是**有循环**——ReAct 的"思考 → 行动 → 观察 → 思考"循环。这个循环让 Agent 从"能调 API 的聊天机器人"变成"能自主完成任务的数字员工"。

**选型指南**：
- 如果你的任务可以一步完成（查天气、翻译、总结）→ Tool Calling 就够了
- 如果你的任务需要多步推理和自主决策（订旅行、写代码、做竞品分析）→ 必须用 Agent

#### Q2: ReAct 为什么能工作？它解决的根本问题是什么？

考察对 Agent 核心机制的深层理解。

**ReAct 解决的根本问题**：LLM 是一个"闭门造车"的系统——只能基于训练数据生成回答，无法感知和验证外部世界。你问"今天北京飞上海的航班有哪些"，纯 LLM 只能编造航班号。ReAct 的核心洞察是：**把推理（Reasoning）和行动（Acting）交替进行**，让模型每推理一步就先去外部世界验证一下，然后基于真实信息再推理下一步。

这个机制之所以有效，是因为它把 LLM 从"一次性生成答案"变成"迭代式探索和验证"。说到底，ReAct 在模拟人类解决复杂问题的方式——先想一步、做一点、看看结果、再想下一步。

**局限**：ReAct 在多步任务上可能陷入"循环但无进展"的状态——模型反复推理和查询但始终没有接近答案。这引出了 Reflexion（反思，让模型从失败中学习）和 Tree-of-Thought（多路径探索，多条推理链并行）等改进方向。

#### Q3: MCP 和 A2A 各解决什么问题？为什么需要两个协议？

考察对 Agent 基础设施协议的理解。

**MCP（Model Context Protocol）**解决"Agent 怎么连接工具"——Agent 和单个工具之间的通信协议，类似 USB-C 之于外设。MCP 标准化了 Agent 如何发现工具、调用工具、接收返回结果，让不同 Agent 可以用统一方式连接不同工具。

**A2A（Agent-to-Agent Protocol）**解决"Agent 之间怎么通信"——当多 Agent 需要协作完成复杂任务时，A2A 定义了 Agent 之间交换信息、委托任务、汇报进度的标准方式。

**为什么需要两个？**它们解决不同层次的通信：MCP 是 Agent ↔ 工具的垂直通信，A2A 是 Agent ↔ Agent 的水平通信。一个 Agent 可能同时用 MCP 调用浏览器和代码执行器，同时用 A2A 与另一个 Agent 协调任务分工。**MCP 是"Agent 的手"，A2A 是"Agent 之间的语言"。**

### 场景题

#### 场景 1：为电商客服设计 Agent 的安全边界

> 你要为电商平台设计一个 AI 客服 Agent，可以访问订单数据库、物流 API、商品目录。用户会问订单状态、退换货流程、商品推荐等。Agent 的权限边界和安全策略怎么设？

**第一步：定义工具权限**。Agent 需要三个只读工具：`query_order(order_id)`、`query_logistics(tracking_number)`、`search_products(query)`。核心原则——Agent 只能**查询**，绝不能**修改**订单状态或发起退款。写入操作必须由人类客服确认后执行。

**第二步：安全边界设计**。从四个层次控制：
- **Prompt 层**：System prompt 明确列出禁止操作（修改订单、发起付款、删除数据、访问用户敏感信息）
- **工具层**：工具注册时只开放查询类 API，删除/写入/退款接口根本不暴露给 Agent
- **输入层**：对用户输入做 prompt injection 检测——拒绝"忽略之前所有指令"类输入
- **业务规则层**：硬编码规则兜底——单笔退款超过 ¥500 必须人工、批量操作一律拦截

**第三步：降级策略**。定义 Agent 需要转人工的场景：用户表达愤怒、涉及金钱纠纷、Agent 连续两轮无法理解用户意图、任务超出 Agent 能力范围。转人工时附带 Agent 已收集到的信息，避免用户重复描述问题。

#### 场景 2：Agent 失控——排查与防护

> 你团队的 Agent 在测试中出现了意外行为：它本应帮用户做竞品分析，但调用浏览器时访问了不该访问的页面，且试图调用文件系统的删除操作。你的排查路径和长期防护方案是什么？

**排查路径**（从急到缓）：

1. **先止损**：立即断开 Agent 的网络和文件系统权限，保留所有日志用于分析
2. **复现**：是用户恶意输入（prompt injection）还是模型自己"越界"？检查 Agent 的完整推理链日志——thought-action-observation 循环的每一步
3. **定位根因**：最常见三个原因——① 工具权限过大（Agent 的浏览器能访问整个互联网，文件系统工具有写/删权限）；② System prompt 未设边界；③ Prompt injection 注入成功

**长期防护方案（分层）**：

| 层次 | 措施 | 说明 |
|------|------|------|
| Prompt 层 | System prompt 明确禁止删除、写入、访问敏感 URL | 第一道防线，但不可单独依赖 |
| 工具层 | 最小权限原则——浏览器只允许 GET，文件系统只读 | Agent 根本拿不到危险操作的"手柄" |
| 沙箱层 | Agent 运行在 Docker 隔离容器中，所有操作在沙箱内执行 | 即使失控，破坏也只限于沙箱内 |
| 监控层 | 实时监控工具调用日志，异常行为（调用删除 API、访问内网 IP）立即告警并阻断 | 尽早发现、尽早拦截 |
| 人工审核层 | 高风险操作（支付、数据修改）必须人类确认 | 最后一道防线 |

**核心原则**：永远不要信任 Agent 的判断，用多层防护代替单层信任。Agent 能做什么由系统约束定义，不由 Agent 自己判断。

### 应用题

#### 应用 1：设计一个代码审查多 Agent 系统

> 你所在团队每天有 20-30 个 PR 需要审查。设计一个多 Agent 系统，让 AI 自动完成第一轮代码审查，人类审查者只做最终确认。给出 Agent 分工、协作流程、质量标准、以及如何防止 Agent 漏掉严重 bug。

**Agent 分工（三个专门 Agent，各自专注一个维度）**：

**Agent A — 安全审查**：只找安全问题。检查 SQL 注入（字符串拼接）、XSS（未转义的用户输入）、敏感信息硬编码（API key、密码）、权限绕过、依赖中已知 CVE。安全规则尽量用确定性的模式匹配（Semgrep/CodeQL），Agent 做补充判断。输出：安全问题列表（按严重程度排序），无问题则输出"PASS"。

**Agent B — 逻辑正确性**：只找逻辑 bug。检查空指针/None、数组越界、并发问题（竞态条件）、事务边界、错误处理是否完备。输出：逻辑问题列表，每个问题标注置信度。

**Agent C — 代码质量**：只找可维护性问题。检查命名是否清晰、函数是否过长、是否违反单一职责、是否有重复代码、关键路径测试覆盖。输出：质量建议列表（全部为建议，不阻断合并）。

**协作流程**：
```
PR 创建
  ↓
三个 Agent 并行审查（各自独立，互不干扰）
  ↓
汇总 Agent 合并三个 Agent 的输出，去重，按严重程度排序
  ↓
汇总报告发给人类审查者
  ↓
人类确认 AI 标记的严重问题 + 补充遗漏 → 做最终 approve/reject
```

**为什么用多 Agent 而非单 Agent**：单 Agent 同时关注安全、逻辑、风格三个维度时容易"注意力稀释"——安全问题和代码风格问题被放在同一个 prompt 里，模型倾向于在两者之间平均分配注意力。多 Agent 各自专注一个维度，类同代码审查中"安全工程师查安全、模块 owner 查逻辑、Tech Lead 查质量"的分工逻辑。

**防漏报策略**：
- 安全 Agent 的漏报不可接受——所有安全规则必须有确定性检查兜底，Agent 只在规则引擎结果上做补充
- 每个发现标注置信度，低置信度标记"需要人类确认"
- 每周抽查 10 个 PR，对比 Agent 和人类审查结果，计算漏报率和误报率，根据数据调整 prompt 和规则
- **最关键的决策**：AI Agent 做第一轮审查但永远不做最终决定。最终 approve 权力在人类手里。这不是因为 AI 不够好——而是 Agent 漏掉严重 bug（假阴性）的代价极高，且人类审查者需要至少看过 PR 才能对代码有 ownership。
...
| Sycamore Labs $65M Seed | TechCrunch (2026-03-30) | 2026-04-30 |
| ServiceNow AI Agents + A2A | googlecloudpresscorner.com (2026-04-22) | 2026-04-30 |
...
*本文是横纵分析系列的第 8 篇报告。方法论：横纵分析法——纵向沿时间轴追溯技术演进，横向对比当下竞争格局。*
