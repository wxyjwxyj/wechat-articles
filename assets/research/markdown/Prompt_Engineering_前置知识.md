...
>
> 🎯 **读完这篇你能**：针对不同任务选择合适的提示策略（Zero-shot / Few-shot / CoT），写出结构清晰的 System Prompt 和多示例 Few-shot 模板，让模型在特定场景下稳定输出符合预期的结果。
...
Few-shot 背后更本质的概念。模型不需要更新参数（重新训练），仅仅在 prompt 中看到几个示例，就能"学会"执行新任务。这是 GPT-3（Brown et al., 2020）最重要的发现，也是 Prompt Engineering 存在的理论基础。
...
CoT 的惊人之处在于：仅让模型"一步一步思考"，就能大幅提升数学、逻辑、多步推理任务的表现。这个能力只在大型模型（>100B 参数）中出现——小模型"不会"逐步推理（Wei et al., 2022）。
...
### Graph-of-Thought（思维图）—— GoT

GoT（Besta et al., 2023）把推理从"树"进一步泛化为"图"。树的分支不能合并——你选了路线 A 就不能把路线 B 的成果拿过来用。图的节点之间可以任意连接：Agent 同时探索 3 种方案，发现方案 A 的第 2 步和方案 B 的第 3 步可以合并，形成更好的方案 C。排序任务上，GoT 比 ToT 质量提高 62% 的同时成本降低了 31%。

走向是：**CoT（一条线）→ ToT（一棵树，可回溯）→ GoT（一张网，可合并）。**
...
## 五、自动优化 prompt：从"梯度搜索"到"编译器"

手写 prompt 很累，能不能让机器自动找最优 prompt？这个方向有两条技术路线。

### Soft Prompt / Prefix-Tuning（软提示 / 前缀调优）—— 用梯度"算"出 prompt

2021 年，Li & Liang 的 **Prefix-Tuning** 和 Lester et al. 的 **Prompt Tuning** 走了一条完全不同的路：**不在输入层写自然语言，而是在输入前面加一串可学习的连续向量（不是人懂的词，是模型懂的数学表示）**，然后用梯度下降优化这些向量。

打个比方：普通 prompt 是你用中文对模型说"请翻译成英文"，Soft Prompt 是你直接调模型内部的"旋钮"，找到一组让翻译效果最好的隐藏信号。这组信号对人来说是一堆数字，但对模型来说比任何自然语言指令都精准。

**局限**：Soft Prompt 需要访问模型内部参数（不是所有 API 都开放），且优化出来的"向量 prompt"完全不透明——你不知道它到底"说了"什么。它更像微调的一种变体，和后来主流的"自然语言 prompt 工程"走了不同的路。但它证明了**自动搜索最优 prompt 是可行的**——这为后来的 DSPy 埋下了伏笔。

### DSPy——用"编译器"自动搜索最优 prompt

DSPy（Khattab et al., 2023）继承了"自动优化 prompt"的思想，但换了一种方式：**不碰模型参数，而是用自然语言搜索**。你声明任务目标，DSPy 自动生成候选 prompt → 测试 → 选最优。
...
DSPy 的核心思想是：prompt 是人跟模型之间文本协议的一种"编译"问题——就像 C 编译器把你的代码变成机器码，DSPy 把你的任务描述变成最优 prompt。到 2026 年已有 34k GitHub Stars（社区估算），是"prompt 工程化"方向的代表性工具。
