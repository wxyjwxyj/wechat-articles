...
简单类比：**蚂蚁个体很简单，蚁群却很聪明**。单个参数不过是一个数字，但 1750 亿个参数（GPT-3 的参数量）（Brown et al., OpenAI, 2020）组合在一起，涌现出了理解、推理、编程等复杂能力。这就是为什么大模型"突然变聪明"——不是有人教了新东西，而是规模带来了质变。

### 上下文窗口（Context Window）

模型一次能"看到"多少内容。上下文窗口的演进是 LLM 发展的一条重要副线：

| 模型 | 时间 | 上下文窗口 | 相当于 |
|------|:----:|:---------:|--------|
| GPT-1 | 2018 | 512 token | 一段话 |
| GPT-2 | 2019 | 1,024 token | 一页纸 |
| GPT-3 | 2020 | 2,048 token | 一篇短文 |
| GPT-4 | 2023.03 | 8K / 32K | 一个章节 |
| Claude 2 | 2023.07 | 100K | 一本书 |
| GPT-4 Turbo | 2023.11 | 128K | 一本半书 |
| Gemini 1.5 Pro | 2024.02 | 100 万 token | 十几本书 |
| Llama 4 Scout | 2025.04 | 1000 万 token | 上百本书 |

六年时间从 512 涨到 1000 万，翻了近两万倍。到 2026 年，1M 已是旗舰模型标配。

### Transformer

2017 年 Google 在论文 "Attention Is All You Need"（Vaswani et al., 2017）中提出的神经网络架构。主流商用 LLM（GPT、Claude、Gemini、Llama、DeepSeek、Qwen）都基于它。另有 Mamba 等 SSM 架构试图替代 Transformer，但目前体量和影响力远不及。

**注意力机制（Attention）** 是 Transformer 的核心创新。它不是对整个句子做一次处理，而是**每个 token 各自算一次**。每个 token 被拆成三个角色：

- **Q（Query，查询）**：代表"我在找什么"
- **K（Key，键）**：代表"我是什么"
- **V（Value，值）**：代表"我有什么内容可以提供"

以一个简单句子"我/爱/北京"（3 个 token）为例，以 token "爱" 的视角走一遍计算过程：

```
1. "爱"的 Q 分别跟三个 token 的 K 做点积（对应位置相乘再求和），得到 3 个原始分数：[0.1, 0.3, 0.6]
2. 对这三个分数做 softmax，转成百分比权重：[10%, 25%, 65%]（加起来 = 100%）
3. 用这些权重去加权求和三个 V："我"×10% + "爱"×25% + "北京"×65%
4. 得到的新向量就是"爱"在这一轮 Attention 后的表示
```

"爱"最终被"北京"影响了 65%，被"爱"自己影响了 25%，被"我"只影响了 10%——模型由此学会了"爱"跟"北京"关系最紧密。句子里每个 token 都独立走一遍这个流程，各产生一个新表示。

**Self-Attention（自注意力）**：以上所有 Q、K、V 都来自同一个句子内部——每个词都在问其他所有词"你跟我的关系是什么"。比如"小明把苹果给了小红，**她**很开心"——当处理 token"她"时，它的 Q 扫过整句，发现"小红"的 K 匹配度最高（都是女性），而"苹果"的 K 匹配度很低（苹果不是人）。所以"她"正确指代了"小红"。传统模型只能看前文，Self-Attention 同时看整句话的所有位置，这是 Transformer 能理解复杂上下文的关键。

### KV Cache（KV 缓存）

回忆上文：每个 token 进来后要经过 W_K、W_V 投影才能产生 K 和 V。实际大模型推理分两段，走的是同一套 Transformer，只是执行模式不同：

**Prefill（预填充）**——用户的问题（prompt）一次性进来，所有 token 并行算 Self-Attention，K、V 全部存进 Cache。同时取最后一个位置的输出，预测出回答的第一个 token "我"：
```
用户问："你爱哪个城市？"（5 个 token）
→ 5 个 token 并行做 W_K,W_V 投影 → K₁₋₅,V₁₋₅ 全部进 Cache
→ 取最后位置的输出 → 预测出 "我"
```

**Decode（逐字生成）**——Cache 里已有 prompt 的全部 K、V，回答部分一个一个 token 出：
```
生成"我"：→ 投影得 K₆,V₆
         → Q₆ 跟 Cache 里 [K₁...K₆] 做注意力 → 输出"爱"
         → K₆,V₆ 追加进 Cache

生成"爱"：→ 投影得 K₇,V₇
         → 从 Cache 取 K₁...K₆        ← prompt 的 K、V 也在里面
         → Q₇ 跟 [K₁...K₇] 做注意力 → 输出"北京"
         → K₇,V₇ 追加进 Cache
...
一句话：**每个 token 投影一次，K、V 存起来，后面直接用，不再投影**。Cache 省的是 W_K、W_V 投影这一步，不是后续的注意力计算。这也是长上下文为什么贵——Cache 越长，显存占用越大。
