...
> 🎯 **读完这篇你能**：根据应用场景选择 Pipeline / DAG / Agent 架构模式，判断何时引入 RAG 和 ReAct 推理循环，并为一个简单需求选型合适的 LLM 框架。
...
**类比**：就像工厂里的流水线——原材料（用户问题）从一头进去，经过多个工序（检索→增强→生成），成品（答案）从另一头出来。每一步都是固定的，顺序不变。

框架让你用声明式的方式定义这个流程，不需要手写每一步的代码。

### DAG（Directed Acyclic Graph，有向无环图）

当流程不是简单的一条线，而是有分支、有并行的复杂结构时，就用到 DAG。

**类比**：Pipeline 像做菜时按顺序放调料（先放油→再放葱→再放菜），DAG 像同时煮着米饭、炒着菜、炖着汤——三条线并行，最后一起上桌。

比如一个知识库机器人可以"同时搜索文档库和数据库 → 把两个结果合并 → 发给 LLM"，这就是一个 DAG。现代 LLM 框架的内部编排引擎大多基于 DAG 设计。
...
**类比**：Pipeline 像自动售货机——投币（输入），掉出可乐（输出），固定流程。Agent 像一个人类店员——你说了"我要喝点什么"，他会自己判断你应该喝可乐还是咖啡，去拿，回来给你，不够凉还会去加冰。

Agent 框架处理这个循环的编排逻辑（状态管理、容错、决策机制）。

### ReAct（推理+行动）

Agent 最核心的工作模式。由 2022 年 Yao et al. 提出（Yao et al., 2022），核心思想是"边思考边行动"：
...
**类比**：就像给你一个助理，不仅给他一张嘴（LLM），还给了他书包里的各种工具——计算器、电话本、文件柜。他平时用不上，但需要时伸手就能拿到。

### Memory（记忆）

多轮对话需要记忆。框架提供：
- 短期记忆（当前会话的对话历史）——类比：你记得住刚才聊了啥
- 长期记忆（跨会话的关键信息）——类比：隔一周回来，还记得你的名字和偏好
...
Anthropic 在 2024 年底推出的开放协议（Anthropic, 2024），统一 Agent 与外部工具的连接方式，被称为"AI 的 USB-C"。主流框架（LangChain、Dify）都已原生支持 MCP，使其成为 Agent-工具连接的标准化接口。
...
| 框架 | 一句话定位 | 谁在用 |
|------|-----------|-------|
| **LangChain** | 通用编排框架，700+ 集成（LangChain Inc.），生态最大 | AI 工程师、Agent 开发者 |
...
Dify 在 2026 年达到 140k GitHub Stars（LangGenius）（超过了 LangChain），说明**低代码/无代码不是"玩具"**。它的增长来自：
...
```
   ...
   Kernel
    ...
    低代码爆发 → Dify 140k stars 超越 LangChain（LangGenius）
    MCP 协议 → 框架都在适配
