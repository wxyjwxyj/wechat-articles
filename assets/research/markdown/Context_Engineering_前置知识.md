...
**直觉类比**：就像做笔记而不是每次从头听课——你在听一节课时（生成文本），记下关键点（KV Cache），后面需要引用时就翻笔记，不需要重新听一遍。笔记记得好，后面查得就快；笔记占地方，就是 KV Cache 占用显存。Context Engineering 中的很多压缩技术本质上就是"怎么让笔记更高效且不丢失关键信息"。
