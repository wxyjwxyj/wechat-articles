...
两种主流方式：
- **图编排**（LangGraph）：用有向图定义步骤和分支。**类比**：像画流程图——你先画好 A→B→C 的路线图，代码自动按图执行。
- **可视化编排**（Dify）：拖拽节点，用箭头连线。**类比**：像用 PPT 画架构图——拖框框、画箭头，不写代码也能定义流程。

选择哪一种取决于团队——开发团队偏爱图编排（灵活可控），业务团队偏爱可视化编排（无需编程）。
...
**类比**：就像飞机的黑匣子——不仅记录"飞机在飞"（传统监控），还记录"飞行员在想什么、按了什么按钮、仪表盘读数怎么变的"（Agent 的行为轨迹）。没有可观测性的 Agent 像个黑箱——出了问题你完全不知道它做了什么决策。
...
**类比**：就像玩一个不能存档的 RPG 游戏——你打了 28 个小时，电脑突然死机了，得从头开始。Durable Execution 就是游戏的自动存档功能——每次关键节点自动保存，任何时候崩溃都能从最近存档恢复。
