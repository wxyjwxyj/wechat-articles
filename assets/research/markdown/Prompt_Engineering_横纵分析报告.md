...
大语言模型在训练中学到的是 token 序列的统计分布——给定上文，预测下一个最可能的词。由此带来三个关键推论：
...
3. **涌现的表现依赖设计的输入**：CoT、ReAct 等推理策略说到底不是"教模型新能力"，而是"通过特定的输入格式释放模型已经具备的能力"。
...
> **📚 关联报告**
> - [AI Agent](./AI Agent_横纵分析报告.md) — ReAct/CoT 等提示策略是 Agent 推理循环的起源
...
**2021 年的 Soft Prompt（连续 prompt）** 走了另一条路：Prefix-Tuning（Li & Liang, ACL 2021）和 Prompt Tuning（Lester et al., arXiv:2104.08691）不在输入层加示例，而是加**可学习的连续向量**。这说到底是用梯度下降优化 prompt，而不是用自然语言写 prompt。它在学术上有影响，但与后来主流的"自然语言 prompt"不同路——它更接近微调的一种变体。
...
### 5.8 2026 年 4-5 月：自动化与"Prompt 已死"的再争论

**GEPA（ICLR 2026）**——Reflective Prompt Evolution——是 2026 年自动 prompt 优化方向最受关注的工作。GEPA 不依赖梯度信息，而是用自然语言反思来迭代优化 prompt：用当前 prompt 测试 → LLM 反思输出质量 → 生成改进版的 prompt → 重复。在多个基准上，GEPA 的优化效果超越了基于 RL 的方法，且不需要训练数据标注。这证明了**"用反思代替梯度"在 prompt 优化中是可行的**。

但同期发表的**"Prompt Optimization Is a Coin Flip"（2026.04）** 给出了清醒的警告：在组合 AI 系统（Compound AI Systems）中，自动 prompt 优化的效果高度不稳定——有时优化后性能反而下降，有时优化对某一指标改善但对另一指标恶化。结论是：自动 prompt 优化不是万能药，需要针对具体任务做诊断判断。

**"Prompt → Context → Harness"三层演进的行业共识**在 2026 年 4-5 月进一步巩固。Prompt Engineering 不会消失——它被 Context Engineering 包含，又在 Harness Engineering 框架下找到了新的位置：prompt 是 Harness 中最底层、最灵活的控制点。
