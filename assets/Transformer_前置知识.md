...
>
> **撰写时间**：2026-04-29  |  **最后更新**：2026-04-29
...
18. [一张图谱串起来](#十八一张图谱串起来)
19. [与 MoE 前置知识的对照](#十九与-moe-前置知识的对照)
20. [延伸阅读](#二十延伸阅读)
...
- **模型 = 一堆可调整的数字（参数），按层级组织，把输入变成输出。** 就像一个巨大的函数：输入中文，输出下一句中文
- **训练 = 不断微调这些数字。** 每次给模型看数据→算出预测→对比标准答案（算 loss）→往 loss 变小的方向调整每一个参数。重复几十亿次
- **推理 = 不再调整参数，只做前向计算。** 输入→层层计算→输出。不调数字，只算结果
...
| 术语 | 大白话解释 |
|------|-----------|
| **模型** | 一堆可调整的数字（参数），按层级组织，把输入变成输出。LLaMA 70B = 700 亿个数字、80 层堆叠 |
...
这个视角跟数据库检索一模一样：Query 是搜索词，Key 是文档索引，Value 是文档内容。跟 MoE 的 Router 也很像——MoE 用门控网络决定"每个 token 去哪几个 expert"，Transformer 用 QK 相似度决定"每个 token 关注哪几个 token"。都是**软路由**，只是路由的对象不同。
...
Google 发现：如果只做理解（分类、问答、NER），Decoder 是多余的。把 Decoder 砍掉、只留 Encoder，用双向上下文做 Masked Language Model 预训练——遮一个词，从左右两个方向同时猜。
...
**为什么最终胜出？** 2018-2020 年，BERT 系在理解任务上完胜（GLUE 榜单刷屏），GPT 系在生成任务上默默积累。但到了 2020 年 GPT-3 证明了——一个足够大的 Decoder-Only 模型，不需要 fine-tune 就能完成分类、翻译、摘要等理解任务。生成是更高层级的元能力。
...
**Pre-LN vs Post-LN**：Post-LN（原始论文）中梯度路径更复杂，深层网络容易梯度爆炸。Pre-LN（LN 放在子层前面）让残差分支有干净的恒等映射路径——现在所有大模型都用 Pre-RMSNorm。
...
- **v2（2023）**：改进并行化策略，A100 FP16 达到 ~230 TFLOPS（约 70% 理论峰值）
- **v3（2024）**：深度适配 H100 的 Hopper 架构特性（WGMMA 指令、TMA 异步拷贝），FP16 达到 740 TFLOPS、FP8 达到 ~1.2 PFLOPS
