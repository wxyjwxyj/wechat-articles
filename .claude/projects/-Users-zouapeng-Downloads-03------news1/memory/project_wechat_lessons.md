...
## 十七、Topic Research Hub 完善 + daily_run 微信重试（2026-04-06 晚）

### ✅ Research Hub 第6数据源：本地公众号文章

新增 `research/wechat_search.py`，从 `content.db` 按关键词搜索已采集的微信文章：
- SQL：`WHERE source_type='wechat' AND (title LIKE :kw OR summary LIKE :kw OR tags LIKE :kw)`
- 返回：title, url, author, summary（截断200字），tags（JSON解析），published_at（仅日期）
- topic_searcher.py 增加第6并行数据源（max_workers=6）
- result_renderer.py 增加 `_render_wechat()` 函数和统计徽章

### ✅ daily_run.sh 微信登录过期自动重试

之前行为：`exit=11` 时采集失败，直接跳过微信。
现在行为（v19）：
1. 检测到 exit=11 → 发 macOS 通知"请扫码登录"
2. `open -a "Google Chrome" "https://mp.weixin.qq.com"` 自动打开登录页
3. `sleep 60` 等待用户扫码
4. 重试 `fetch_wechat_today.py`
5. 仍然失败才记录错误继续
...
| 问题 | 说明 |
|------|------|
| exit code 78 | `EX_CONFIG`，表示 launchd 配置问题或脚本从未开始运行 |
| 错过的时间点不补跑 | launchd `StartCalendarInterval` 不重试，机器睡眠期间的任务直接跳过 |
| plist 时间核查 | 通过 `cat ~/Library/LaunchAgents/com.news1.daily.plist` 确认，当前是 **11:30**（不是 9:57）|
| 日志路径 | `.claude/logs/YYYY-MM-DD.log`，每次 daily_run.sh 都追加写入 |

### ✅ Claude 评分器加主题相关性约束

`claude_scorer.py` 的 `score_resources()` 加了 `topic` 参数：
- prompt 中明确："如果资源与搜索主题不直接相关，无论质量多高，都必须打 1-2 分"
- 解决 GitHub 全文搜索导致高 star 无关项目（如 JavaGuide）混入结果的问题

### ✅ doc_library.py 扩充 Agent 相关文档

搜索 "Agent" 原来只返回 1 条（LangChain），现在返回 10 条。
新增框架：LangGraph、AutoGen（Microsoft）、CrewAI、OpenAI Assistants API
加 "agent" tag：LlamaIndex、OpenAI、Anthropic、stable-baselines3、ray-rllib、gymnasium

### 项目版本更新

```
v19: daily_run.sh 微信登录过期自动打开 Chrome + 60s 重试
v20: Topic Research Hub 新增第6数据源（本地公众号文章）
     + doc_library 扩充 Agent 框架
     + Claude 评分器加主题相关性约束
```
